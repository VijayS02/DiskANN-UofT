...
const HistogramPlot: React.FC<HistogramProps> = ({ title, data, xLabel, yLabel, yLog }) => {
    ...
    const filteredData = data.filter((value) => 
        (xMin === undefined || value >= xMin) && (xMax === undefined || value <= xMax)
    );
...
function RenderGraph({
    ...
    const metadata = graphData["metadata"] as { type: string; props: any } | undefined;
    ...
    return <div key={graphId}> 
    <GraphComponent key={graphId} {...props} />
    </div>
}


export default function MsgPackGraphRenderer({ graphIds, data }: { graphIds: string[], data: Record<string, unknown> }) {
