    ...
    <div className="w-full border-2 border-gray-200 p-3 py-5 rounded-md">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <FileChooser
          ...
          setSelectedFile={setQueryFile}
          placeholder="Select a query file..."
        />
        <div>
          ...
          <Input
            ...
            value={lQuery}
            onChange={(e) =>
              setLquery(e.target.value ? parseInt(e.target.value) : "")
            }
          />
        </div>
        <div>
          ...
          <Input
            ...
            value={k}
            onChange={(e) =>
              setK(e.target.value ? parseFloat(e.target.value) : "")
            }
          />
        </div>
        <SelectMetrics
...
function IndexSelector({
  ...
  const { data, loading, error } = useFetch<{ indexes: Index[] }>(
    "/list_indexes",
    "GET",
    undefined,
    {
  ...
  return (
    <div>
      ...
      <Select
        disabled={loading || data?.indexes.length == 0}
        value={selectedIndex}
...
function SelectMetrics({ selectedMetrics, setSelectedMetrics }: any) {
  const { data, loading, error } =
    useFetch<Record<string, { label: string }>>("/query_metrics");
  ...
  useEffect(() => {
    // If data is loaded set default to no metrics
    if (data) {
      setSelectedMetrics([]);
    }
  }, [data]);
        ...
        <div className="space-y-3 grid grid-cols-2 gap-2 my-2">
          {data &&
            data.results
              ...
              .map((index, i) => <ResultView key={i} result={index} />)}
        </div>
      }
    </div>
  );
...
function ResultView({ result }: { result: ResultInfo }) {
  ...
  return (
    <div className="border-2 border-gray-200 p-3 py-5 rounded-md">
      <div className="flex">
        <div className="text-lg font-bold">
          {moment(result.time * 1000).fromNow()}
        </div>
        <Button size="sm" className="ml-auto" asChild>
          <Link href={`/query/${result.id}`}>View</Link>
        </Button>
      </div>
      <div className="grid-cols-2 grid gap-x-1 gap-y-3 mb-4">
        ...
        <Stat
          name="Query File"
          value={`${result.query_file} ${
            result?.n ? "(" + result?.n?.toLocaleString() + ")" : ""
          }`}
        />
